...
const journeyItems = [
  ...
  { title: "Indoor Sports", icon: PiBarbell },
  { title: "Martial Arts", icon: PiShield },
  { title: "Outdoor Sports", icon: PiSoccerBall },
  { title: "Swimming", icon: PiWaves },
  { title: "Reverse Engineering", icon: PiWrench },
  { title: "Food Science", icon: PiFlask },
  { title: "Islamic Garden & Nature", icon: PiLeaf },
  { title: "Prayer / Masjid", icon: PiMosque },
  { title: "Personality Development", icon: PiMicrophoneStage },
...
export default function LearningJourneyProgressDemo2({
  ...
  return (
    <div className="right-10 top-1/2 z-[850] hidden h-fit -translate-y-1/2 items-center gap-3 rounded-lg bg-white/50 px-3 py-4 backdrop-blur-md md:fixed md:flex">
          ...
          return (
            <div key={item.id} className="group relative">
              <button
                ...
                className={`peer grid md:size-6 lg:size-8 cursor-pointer place-items-center rounded-full border transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#19357a]/45 focus-visible:ring-offset-2 ${
                  isActive
              ...
              <span
                id={tooltipId}
                role="tooltip"
                className={`pointer-events-none absolute right-full top-1/2 z-20 mr-3 -translate-y-1/2 whitespace-nowrap rounded-md border border-white/10 bg-[#071839]/92 px-3 py-2 font-sans text-[clamp(0.4rem,1vw+0.3rem,0.6rem)] font-semibold uppercase tracking-[0.12em] text-white shadow-[0_10px_28px_rgba(3,15,40,0.24)] backdrop-blur-md transition-all delay-100 duration-200 ease-out group-hover:delay-0 peer-focus-visible:delay-0 ${
                  isActive
      ...
      <div className="relative md:h-[380px] lg:h-[490px] w-px bg-[#344384]/18">
        <span
