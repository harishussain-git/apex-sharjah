...
const animationStyle = (animation, progress, leaving = false) => {
  ...
  if (animation === "zoom-in") {
    const blur = leaving ? progress * 10 : (1 - progress) * 10
    const isSettled = leaving ? progress === 0 : progress === 1

    return {
      opacity: leaving ? 1 - progress : progress,
      transform: isSettled
        ? "none"
        : `translate3d(0, ${leaving ? progress * -24 : (1 - progress) * 24}px, 0) scale(${leaving ? 1 + progress * 0.38 : 1.08 - progress * 0.38})`,
  ...
  // - 0.08 = scale amount
  // - 8 = blur amount
  const blur = leaving ? progress * 8 : (1 - progress) * 8
  const isSettled = leaving ? progress === 0 : progress === 1
  ...
  return {
    ...
    transform: isSettled
      ? "none"
      : `translate3d(0, ${leaving ? progress * 18 : (1 - progress) * 18}px, 0) scale(${leaving ? 1 - progress * 0.08 : 1.08 - progress * 0.08})`,
    filter: blur > 0 ? `blur(${blur}px)` : "none",
  }
}
...
export function getSequenceLayerStyle(anchor, frame) {
  ...
  if (anchor.enter && frame < anchor.enter.to) {
    return {
      ...animationStyle(anchor.enter.animation, rangeProgress(frame, anchor.enter.from, anchor.enter.to)),
      pointerEvents: "none",
    }
  }
